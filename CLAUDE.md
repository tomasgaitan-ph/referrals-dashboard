...
- **Detalle:** la card "Deal" muestra **Settlement date** y el **Stage** legible ("Pre-settlement" para `257909958`); la card "Referrer payment" habilita el botón según el flag `canMarkReferrerPaid` que calcula d2 (ver "Guard del botón"). Cuando está deshabilitado muestra los **requisitos del programa** (display-only, derivados de `deal.product_choice` vía `paymentRequirements()` en `program.js`): SP → "Deal in Pre-settlement" + "Real Settlement Date set"; VH/IH → "Investment Ticket in Closed Won" (sin Real Settlement Date — el back NO gatea por `real_settlement_date` en VH/IH); sin programa → fallback genérico. **No** recalcula elegibilidad (eso sigue en `canMarkReferrerPaid`); es solo texto explicativo. Cuando ya está pagado muestra el estado "completed" sin botón.
...
- **Documentación técnica (Drive) — update a v1.3 en curso (2026-06-30):** la doc canónica en Google Drive (*"Documentación Técnica del Sistema de Referrals — PropHero"*, file ID `1OrXzK5Ub5VJkaiMUzUkpeDZNAGLBi8lr`, hoy v1.2) se está actualizando a v1.3 con los cambios de jun 2026. **Lado dashboard ya hecho** (redlines listos para pegar): Parte 2 (detalle read-only + Mark as paid manual, baja de edición fiscal, programa desde `product_choice` con SP/VH/IH e IreHero, filtros client-side, "Settlement period", KPI "Ready to transfer", RefreshReminder), 4.3 (`product_choice` + `real_settlement_date` en Deal), APIs del dashboard en 4.5 (alta de `dashboard-mark-referrer-paid`, baja de `dashboard-update-referrer-fiscal`), Parte 5 (`VITE_N8N_ENV_SUFFIX`) y porción front del changelog v1.3. **Handoff enviado al agente de `referrals-n8n`** para su mitad (Parte 1 actores/ciclo/estados, 4.5 flujos, 4.9 auth) + 8 confirmaciones (Flujo 03, wf-d6, objeto Investment Ticket de VH/IH, IreHero/`product_choice`, descuento SP-only vs VH/IH, wf-d5, WF-E, Error Handler). **Pendiente:** recibir los redlines de n8n, mergear el changelog v1.3 y **aplicar todo a mano en el .docx** (las tools del MCP de Drive sólo leen, no escriben in-place).
  - **Decisiones de negocio (2026-06-30, confirmadas por Tom tras la verificación del agente n8n en PROD):**
    - **Q1 — BC sin automatización:** la carga de la línea REFERRAL en Business Central y el pago al referrer son **manuales** ahora (finance, vía nota de abono). Ningún workflow activo escribe en BC (Flujo 03 deprecado). → doc: paso 5 del ciclo + §4.6 + §4.7 se reescriben como proceso manual.
    - **Q2 — Descuento al referido:** la intención es **los 3 programas**, pero hoy **solo se cumple para SP**. El registro a nivel objeto Referral (`referido_discount_status`: F02 lo crea `pending`, WF-E lo marca `applied`) es **agnóstico al programa**; la propagación a nivel Deal de Sales (copia del código + `discount_dropdown`, vía WF-A/WF-B) opera **solo en pipeline SP (default)** → un referido que compra VH/IH **no recibe hoy el descuento a nivel deal**. doc: no afirmar "3 programas" sin asterisco; cobertura VH/IH = parte de la expansión VH/IH pendiente.
    - **Q3 — Gate de pago VH/IH:** es **solo Investment Ticket en Closed Won** (pipeline 3741706483 + stage 5232707822); el back **no** chequea `real_settlement_date` para VH/IH (sí para SP). El front estaba desincronizado → **fix aplicado**: se quitó "Real Settlement Date set" de VH/IH en `PROGRAM_PAYMENT_REQUIREMENTS` (`program.js`) + test actualizado.
    - **Q4 — Referrers VH/IH:** los clientes VH/IH **deben poder ser referrers**. Hoy F01 genera `referrer_code` solo para closed-won del pipeline SP → **pendiente de back** (no se implementa en este pase; documentado como aprobado-pendiente).
    - **Expansión VH/IH (epic de back, pendiente):** no es un solo trigger. Abarca (a) F01 → generar código para referrers VH/IH; (b) WF-A/WF-B → propagar código + descuento a deals VH/IH (referidos); (c) verificar que los referidos VH/IH transiten los pipelines que disparan F02 (si no, tampoco hay tracking a nivel Referral). Mientras esté pendiente, el lado VH/IH del programa está parcialmente construido.
