...
- **Documentación técnica (Drive) — update a v1.3 en curso (2026-06-30):** la doc canónica en Google Drive (*"Documentación Técnica del Sistema de Referrals — PropHero"*, file ID `1OrXzK5Ub5VJkaiMUzUkpeDZNAGLBi8lr`, hoy v1.2) se está actualizando a v1.3 con los cambios de jun 2026. **Lado dashboard ya hecho** (redlines listos para pegar): Parte 2 (detalle read-only + Mark as paid manual, baja de edición fiscal, programa desde `product_choice` con SP/VH/IH e IreHero, filtros client-side, "Settlement period", KPI "Ready to transfer", RefreshReminder), 4.3 (`product_choice` + `real_settlement_date` en Deal), APIs del dashboard en 4.5 (alta de `dashboard-mark-referrer-paid`, baja de `dashboard-update-referrer-fiscal`), Parte 5 (`VITE_N8N_ENV_SUFFIX`) y porción front del changelog v1.3. **Handoff enviado al agente de `referrals-n8n`** para su mitad (Parte 1 actores/ciclo/estados, 4.5 flujos, 4.9 auth) + 8 confirmaciones (Flujo 03, wf-d6, objeto Investment Ticket de VH/IH, IreHero/`product_choice`, descuento SP-only vs VH/IH, wf-d5, WF-E, Error Handler). **Pendiente:** recibir los redlines de n8n, mergear el changelog v1.3 y **aplicar todo a mano en el .docx** (las tools del MCP de Drive sólo leen, no escriben in-place).
  - **Decisiones de negocio (2026-06-30, confirmadas por Tom tras la verificación del agente n8n en PROD):**
    ...
    - **Q2 — Descuento al referido:** €500 para **los 3 programas (SP/VH/IH)**, funcionando hoy. **Todo Sales cierra en el pipeline default** y el producto se decide después en OPS (`product_choice`), así que F01 y WF-A/WF-B cubren a todos los clientes sin ramificar por producto (Caso A, confirmado por Tom). No hay "propagación solo SP" ni pendiente. → doc: sacar "solo Spain", sin asterisco.
    ...
    - **Q4 — Referrers VH/IH:** los clientes VH/IH **pueden ser referrers hoy**. Como todo Sales cierra en el pipeline default, F01 genera `referrer_code` para todos los closed-won sin importar el producto → cubre SP/VH/IH. No hay pendiente (Caso A).
