...
- **Detalle:** la card "Deal" muestra **Settlement date** y el **Stage** legible ("Pre-settlement" para `257909958`); la card "Referrer payment" habilita el botón según el flag `canMarkReferrerPaid` que calcula d2 (ver "Guard del botón"). Cuando está deshabilitado muestra los **requisitos del programa** (display-only, derivados de `deal.product_choice` vía `paymentRequirements()` en `program.js`): SP → "Deal in Pre-settlement" + "Real Settlement Date set"; VH/IH → "Investment Ticket in Closed Won" + "Real Settlement Date set"; sin programa → fallback genérico. **No** recalcula elegibilidad (eso sigue en `canMarkReferrerPaid`); es solo texto explicativo. Cuando ya está pagado muestra el estado "completed" sin botón.
